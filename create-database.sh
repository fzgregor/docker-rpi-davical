...
create_db_user() {
  if ! db_users | grep "^${1}$" >/dev/null ; then
    psql -qXAt ${PSQLOPTS} -c "CREATE USER ${1} WITH NOCREATEDB NOCREATEROLE PASSWORD '${2}';" template1
...
create_db_user "${AWL_DBAUSER}" "${AWL_DBAPASS}"
create_db_user "${AWL_APPUSER}" "${AWL_APPPASS}"
...
PGPASSWORD=${AWL_DBAPASS} psql -qXAt ${PSQLOPTS} ${DBA} -f "${AWLDIR}/dba/awl-tables.sql" "${DBNAME}" 2>&1
PGPASSWORD=${AWL_DBAPASS} psql -qXAt ${PSQLOPTS} ${DBA} -f "${AWLDIR}/dba/schema-management.sql" "${DBNAME}" 2>&1

#
# Load the DAViCal tables
PGPASSWORD=${AWL_DBAPASS} psql -qXAt ${PSQLOPTS} ${DBA} -f "${DBADIR}/davical.sql" "${DBNAME}" 2>&1

#
# Set permissions for the application DB user on the database
if ! PGPASSWORD=${AWL_DBAPASS} ${DBADIR}/update-davical-database ${DBAOPTS} --dbname "${DBNAME}" --appuser "${AWL_APPUSER}" --nopatch --owner "${AWL_DBAUSER}" ; then
        cat <<EOFAILURE
* * * * ERROR * * * *
...
PGPASSWORD=${AWL_DBAPASS} psql -qXAt ${PSQLOPTS} ${DBA} -f "${DBADIR}/base-data.sql" "${DBNAME}" 
...
PGPASSWORD=${AWL_DBAPASS} psql -qX ${PSQLOPTS} ${DBA} -c "UPDATE usr SET password = '**${ADMINPW}' WHERE user_no = 1;" "${DBNAME}"
